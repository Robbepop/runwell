...
impl<'a> Export<'a> {
    ...
    pub fn item(&self) -> ExportItem {
        match self.kind {
            ExportKind::Function => {
                ExportItem::Function(FunctionId::from_u32(self.raw_index))
            }
            ExportKind::Table => {
                ExportItem::Table(TableId::from_u32(self.raw_index))
            }
            ExportKind::Memory => {
                ExportItem::Memory(LinearMemoryId::from_u32(self.raw_index))
            }
            ExportKind::Global => {
                ExportItem::Global(GlobalVariableId::from_u32(self.raw_index))
            }
        }
    }
}
